...
resource "aws_security_group" "chono-pub-sg-web" {

    name = "chono-pub-sg-web"

    description = "CHONO Public SG WEB"
...
resource "aws_security_group" "chono-pub-sg-db" {

    name = "chono-pub-sg-db"

    description = "CHONO Public SG DB"
...
resource "aws_instance" "chono-server" {
  ...
  vpc_security_group_ids = [element([aws_security_group.chono-pub-sg-db.id,aws_security_group.chono-pub-sg-web.id],count.index)]
  ...
  provisioner "file" {
    source = "./provision-${count.index + 1}.sh"
    destination = "/tmp/provision-${count.index + 1}.sh"
    connection {
      type = "ssh"
      user = "ubuntu"
      private_key = file("/home/chono/aws-ec2/amazon-ec2-key.pem")
  ...
  provisioner "remote-exec" {
      inline = [
        "chmod +x /tmp/provision-${count.index + 1}.sh",
        "/tmp/provision-${count.index + 1}.sh"
      ]
      connection {
      type = "ssh"
      user = "ubuntu"
